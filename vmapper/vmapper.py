...
class mapper():
    def __init__(self,
        ...
        cube_size[:] = (data_p_max[:]-data_p_min[:]) / \
            (1+(num_cube-1)*(1-overlap))
        cube_min = np.array([[j*cube_size[i]*overlap+data_p_min[i]
                            for j in range(num_cube)] for i in range(Dim)])
        cube_max = np.array([[cube_min[i, j]+cube_size[i]
                            for j in range(num_cube)] for i in range(Dim)])
        if adaptive == True:
            for i in range(Dim):
                ...
                bd[0] = data_p_min[i]
                bd[1:-1] = [0.5*(sorted_val[(j+1)*num_cell_pt] +
                                 sorted_val[(j+1)*num_cell_pt+1]) for j in range(num_cube-1)]
        ...
        data_sub_id_1d = np.array([[np.where((data_p.T[i_dim] >= cube_min[i_dim, i_cube]) & (
            data_p.T[i_dim] <= cube_max[i_dim, i_cube]))[0] for i_cube in range(num_cube)] for i_dim in range(Dim)])
        num_cube_all = num_cube**Dim
        data_sub_id = np.empty(num_cube_all, dtype=list)
        data_sub_id = [data_sub_id_1d[0, i % num_cube]
                       for i in range(num_cube_all)]
        for i in range(num_cube_all):
            for i_dim in range(Dim-1):
                j = int(i/(num_cube**(i_dim+1)) % num_cube)
                data_sub_id[i] = np.intersect1d(
                    data_sub_id[i], data_sub_id_1d[i_dim+1, j])
        data_sub = np.empty(num_cube_all, dtype=list)
        data_sub = [np.array(data[data_sub_id[i]])
                    for i in range(num_cube_all)]
        ...
        for i in range(num_cube_all):
            if len(data_sub[i]) == 0:
                ...
                data_sub_clstr[i] = np.array(
                    clusterer.fit_predict(data_sub[i]), dtype=int)
        ...
        for i in range(num_cube_all):
            if data_sub_clstr_num[i] > 0:
                ...
                for j in list(data_sub_clstr_set[i]):
                    node_id_set[num_node] = np.array(
                        data_sub_id[i][np.where(data_sub_clstr[i] == j)[0]], dtype=int)
                    id_dupl = 0
        ...
        self.adjcy_mat = np.array([[len(np.intersect1d(self.node_id_set[i], self.node_id_set[j]))
                                  for i in range(self.num_node)] for j in range(self.num_node)], dtype=int)
        self.data_p = data_p
    ...
    def out_graph_graphviz(
        ...
        for i in range(self.num_node):
            ...
            G.node(str(i), label='', fillcolor='%s' %
                   vmapper.color.color_dict[self.node_color_id[i_color]][0], width='0.075', shape='circle', style='filled')
        for i in range(self.num_node):
            for j in range(i+1, self.num_node):
                if self.adjcy_mat[i, j] > 0:
                    G.edge(str(j), str(i), arrowhead='none',
                           color='darkslategray')
    ...
    def out_graph_pca(
        ...
        for i in range(self.num_node):
            for j in range(i+1, self.num_node):
                if self.adjcy_mat[i, j] > 0:
                    ...
                    data_edge_pca[i_edge, 2:4] = data_node_pca[j] - \
                        data_node_pca[i]
                    i_edge += 1
        ...
        ax.scatter(data_node_pca[:, 0], data_node_pca[:,
                   1], zorder=1, s=10, color='black')
        ax.quiver(data_edge_pca[:, 0], data_edge_pca[:, 1], data_edge_pca[:, 2], data_edge_pca[:, 3], color='gray', angles='xy',
                  scale_units='xy', width=0.002, scale=1, alpha=0.5, zorder=0, headwidth=0, headlength=0, headaxislength=0)
    ...
    def out_graph_tsne(
        ...
        fig = plt.figure(figsize=(10, 10))
        ax = fig.add_subplot(1, 1, 1)
        ax.scatter(data_node_tsne[:, 0], data_node_tsne[:,
                   1], zorder=1, s=10, color='black')
        ax.quiver(flow[:, 0], flow[:, 1], flow[:, 2], flow[:, 3], color='gray', angles='xy', scale_units='xy',
                  width=0.002, scale=1, alpha=0.5, zorder=0, headwidth=0, headlength=0, headaxislength=0)
    ...
    def out_cytoscape(
        ...
        for i in range(self.num_node):
            ...
            node_info += '{\"data\" : {\"id\":\"%s\",\"member\":\"%s\",\"count\":%d,\"PCA1\":%.5f,\"PCA2\":%.5f,\"tSNE1\":%.5f,\"tSNE2\":%.5f' % (
                self.node_name[i], member, len(self.node_id_set[i]), data_node_pca[i, 0], data_node_pca[i, 1], data_node_tsne[i, 0], data_node_tsne[i, 1])
            for k in range(num_meta):
                if meta_func[0, k] in meta_data[0]:
                    ...
                    meta_data_sec = meta_data[1:, np.where(
                        meta_data[0] == meta_func_name)[0][0]]
                    node_info += ',\"%s[%s]\":%s' % (meta_func[0, k], meta_func[1, k], vmapper.meta_func.add_meta_info(
                        meta_func[0, k], meta_data_sec, meta_func[1, k], self.node_id_set[i]))
            node_info += '}},'
        ...
        for i in range(self.num_node):
            for j in range(i+1, self.num_node):
                if self.adjcy_mat[i, j] > 0:
                    edge_idx_set = np.intersect1d(
                        self.node_id_set[i], self.node_id_set[j])
                    member = '%s' % sample_name[edge_idx_set][0]
                    for k in range(len(sample_name[edge_idx_set])-1):
                        member += ', %s' % sample_name[edge_idx_set][k+1]
                    edge_info += '{\"data\" : {\"source\":\"%s\",\"target\":\"%s\",\"name\":\"%s-%s\",\"member\":\"%s\","count":%d,\"annotation\":\"strict\"' % (
                        self.node_name[i], self.node_name[j], self.node_name[i], self.node_name[j], member, self.adjcy_mat[i, j])
                    for k in range(num_meta):
                        if meta_func[0, k] in meta_data[0]:
                            meta_func_name = meta_func[0, k]
                            if meta_func[1, k][-4:] == 'RECODE':
                            ...
                            meta_data_sec = meta_data[1:, np.where(
                                meta_data[0] == meta_func_name)[0][0]]
                            edge_info += ',\"%s[%s]\":%s' % (meta_func[0, k], meta_func[1, k], vmapper.meta_func.add_meta_info(
                                meta_func[0, k], meta_data_sec, meta_func[1, k], edge_idx_set))
                    edge_info += '}},'
        edge_info = edge_info[:-1] + ']'
        out_cyjs.write('{%s,\"elements\":{%s,%s}}' %
                       (data_info, node_info, edge_info))
    ...
    def out_cytoscape_light(
        ...
        meta_data_out_node = np.empty(
            [self.num_node+1, num_meta+n_default], dtype=object)
        meta_data_out_node[0, 0] = 'name'
        meta_data_out_node[0, 1] = 'member'
        meta_data_out_node[0, 2] = 'count'
        meta_data_out_node[0, n_default:] = ['%s[%s]' % (
            meta_func[0][i], meta_func[1][i]) for i in range(num_meta)]

        meta_data_out_edge = np.empty(
            [num_edge+1, num_meta+n_default], dtype=object)
        meta_data_out_edge[0, 0] = 'name'
        meta_data_out_edge[0, 1] = 'member'
        meta_data_out_edge[0, 2] = 'count'
        meta_data_out_edge[0, n_default:] = ['%s[%s]' % (
            meta_func[0][i], meta_func[1][i]) for i in range(num_meta)]
        ...
        for i in range(self.num_node):
            ...
            for k in range(num_meta):
                if meta_func[0, k] in meta_data[0]:
                    ...
                    meta_data_sec = meta_data[1:, np.where(
                        meta_data[0] == meta_func_name)[0][0]]
                    meta_data_out_node[i+1, k+n_default] = vmapper.meta_func.add_meta_info(
                        meta_func[0, k], meta_data_sec, meta_func[1, k], self.node_id_set[i], id_light=True)
        i_edge = 0
        for i in range(self.num_node):
            for j in range(i+1, self.num_node):
                if self.adjcy_mat[i, j] > 0:
                    edge_idx_set = np.intersect1d(
                        self.node_id_set[i], self.node_id_set[j])
                    sif_data[self.num_node +
                             i_edge] = '%s - %s' % (self.node_name[i], self.node_name[j])
                    meta_data_out_edge[i_edge+1, 0] = '%s (-) %s' % (
                        self.node_name[i], self.node_name[j])
                    member = '%s' % sample_name[edge_idx_set][0]
                    for k in range(len(sample_name[edge_idx_set])-1):
                        member += ', %s' % sample_name[edge_idx_set][k+1]
                    ...
                    for k in range(num_meta):
                        if meta_func[0, k] in meta_data[0]:
                            ...
                            meta_data_sec = meta_data[1:, np.where(
                                meta_data[0] == meta_func_name)[0][0]]
                            meta_data_out_edge[i_edge+1, k+n_default] = vmapper.meta_func.add_meta_info(
                                meta_func[0, k], meta_data_sec, meta_func[1, k], edge_idx_set, id_light=True)
                    i_edge += 1
        np.savetxt('%s/%s.sif' % (out_dir, out_file),
                   sif_data, delimiter=",", fmt="%s")
        np.savetxt('%s/%s_node.tsv' % (out_dir, out_file),
                   meta_data_out_node, delimiter="\t", fmt="%s")
        np.savetxt('%s/%s_edge.tsv' % (out_dir, out_file),
                   meta_data_out_edge, delimiter="\t", fmt="%s")
    ...
    def out_fig_cube_docomp(
        ...
        for j in range(len(self.cube_min[i])):
            ...
            h = 0.5*(u+l)
            ap = self.data_p_min[i+1]-(1+0.75*(j % 2))*2e-2 * \
                (self.data_p_max[i+1]-self.data_p_min[i+1])
            ...
            h = 0.5*(u+l)
            ap = self.data_p_min[i]-(1+0.75*(j % 2)) * \
                2e-2*(self.data_p_max[i]-self.data_p_min[i])
    ...
    def out_graph_graphviz_V(
        ...
        for i in range(self.num_node):
            i_color = i % color_id_max
            G.node(str(i), label='', fillcolor='%s' %
                   vmapper.color.color_dict[self.node_color_id[i_color]][0], width='0.05', shape='circle', style='filled')
        for i in range(self.num_node):
            for j in range(i+1, self.num_node):
                if self.adjcy_mat[i, j] > 0:
                    ...
                    if vel > 0:
                        G.edge(str(i), str(j), arrowhead='normal',
                               arrowsize='0.2', color='darkslategray')
                    else:
                        G.edge(str(j), str(i), arrowhead='normal',
                               arrowsize='0.2', color='darkslategray')
        G.attr(fontsize='8', label=fig_title, dpi='200', layout=fig_layout)
        ...
        for i in range(n_node):
            for j in range(i+1, n_node):
                if self.adjcy_mat[i, j] > 0:
                    ...
                    if vel > 0:
                        data_edge_pca[i_edge, 0:2] = data_node_pca[i]
                        data_edge_pca[i_edge,
                                      2:4] = data_node_pca[j]-data_node_pca[i]
                    else:
                        data_edge_pca[i_edge, 0:2] = data_node_pca[j]
                        data_edge_pca[i_edge,
                                      2:4] = data_node_pca[i]-data_node_pca[j]
    ...
    def out_graph_tsne_V(
            ...
            for j in range(i+1, self.num_node):
                if self.adjcy_mat[i, j] > 0:
                    ...
                    if vel > 0:
                        ...
                        data_edge_tsne[i_edge,
                                       2:4] = data_node_tsne[j]-data_node_tsne[i]
                    else:
                        data_edge_tsne[i_edge, 0:2] = data_node_tsne[j]
                        data_edge_tsne[i_edge,
                                       2:4] = data_node_tsne[i]-data_node_tsne[j]
                    mean_vel = np.mean(data_vel[idx_ij], axis=0)
                    flow = np.dot(mean_vel, rltv_vel)/np.linalg.norm(rltv_vel)
                    edge_flow[i_edge] = flow
                    i_edge += 1
        ...
        ax = fig.add_subplot(1, 1, 1)
        ax.scatter(
            data_node_tsne[:, 0], data_node_tsne[:, 1], zorder=1, s=5, color='gray')
        clim_min = np.percentile(edge_flow, 20)
        clim_max = np.percentile(edge_flow, 80)
        clim_delta = clim_max-clim_min
        ax.quiver(data_edge_tsne[:, 0], data_edge_tsne[:, 1], data_edge_tsne[:, 2], data_edge_tsne[:, 3], edge_flow, cmap='coolwarm', clim=(
            clim_min, clim_max), angles='xy', scale_units='xy', width=0.002, scale=1, alpha=0.8, zorder=0, headwidth=5, headlength=5, headaxislength=5)
    ...
    def out_cytoscape_V(
            ...
            for i in range(n_node):
                for j in range(i+1, n_node):
                    if self.adjcy_mat[i, j] > 0:
                        ...
                        flow = np.sum(
                            v_e/np.linalg.norm(data_vel[idx_ij], axis=1)/np.linalg.norm(rltv_vel))
                        grad_mat[i_edge, i] = -1
                        grad_mat[i_edge, j] = 1
                        flow_vec[i_edge] = flow
                        flow_mat[i, j] = flow
        ...
        for i in range(n_comp_cycle):
            ...
            d_adjcy_mat_i = np.sign(
                (d_adjcy_mat_csr*d_adjcy_mat_csr_i).toarray())
        ...
        for i in range(self.num_node):
            ...
            node_info += '{\"data\" : {\"id\":\"%s\",\"member\":\"%s\",\"count\":%d,\"divergence\":%.5f,\"potential\":%.5f,\"diffusion\":%.5f,\"cycle_score(flow)\":%d,\"cycle_score(div0 flow)\":%d,\"PCA1\":%.5f,\"PCA2\":%.5f,\"tSNE1\":%.5f,\"tSNE2\":%.5f' % (
                self.node_name[i], member, len(self.node_id_set[i]), divergence[i], potential[i], diffusion[i], cycle_score_flow[i], cycle_score_div0[i], data_node_pca[i, 0], data_node_pca[i, 1], data_node_tsne[i, 0], data_node_tsne[i, 1])
            for k in range(num_meta):
                if meta_func[0, k] in meta_data[0]:
                    meta_func_name = meta_func[0, k]
                    if meta_func[1, k][-4:] == 'RECODE':
        ...
        for i in range(n_node):
            for j in range(i+1, n_node):
                if self.adjcy_mat[i, j] > 0:
                    ...
                    ind_vel_prob = 2 * \
                        np.sum(
                            np.abs(v_e[np.sign(v_e) == np.sign(vel)]))/np.sum(np.abs(v_e))-1
                    ...
                    if np.linalg.norm(rltv_vel) > 0:
                        for k in edge_idx_set:
                            if np.linalg.norm(data_vel[k]) > 1.0e-5:
                                pt_flow += np.dot(data_vel[k], rltv_vel) / \
                                    np.linalg.norm(data_vel[k])
...
class smapper():
    def __init__(
        ...
        cube_min = np.array(
            [[j*cube_size*overlap+data_p1_min for j in range(num_cube)] for i in range(d_proj)])
        cube_max = np.array(
            [[cube_min[i, j]+cube_size for j in range(num_cube)] for i in range(d_proj)])
        if adaptive == True:
        ...
        data_sub_id_1d = np.array([[np.where((data_p.T[i_dim] >= cube_min[i_dim, i_cube]) & (
            data_p.T[i_dim] <= cube_max[i_dim, i_cube]))[0] for i_cube in range(num_cube)] for i_dim in range(d_proj)])
        ...
        for i in range(num_cube_all):
            for i_dim in range(d_proj-1):
                ...
                data_sub_id[i] = np.intersect1d(
                    data_sub_id[i], data_sub_id_1d[i_dim+1, j])
        data_sub = np.empty(num_cube_all, dtype=list)
        data_sub = [np.array(data_pca[data_sub_id[i]])
                    for i in range(num_cube_all)]
        data_sub_clstr = np.empty(num_cube_all, dtype=list)
        data_sub_clstr_set = np.empty(num_cube_all, dtype=list)
        data_sub_clstr_num = np.empty(num_cube_all, dtype=int)
    ...
    def out_png(
        ...
        for i in range(self.num_node):
            G.node(str(i), label='', fillcolor='%s' %
                   vmapper.color.color_dict[self.node_color_id[i]][0], width='0.075', shape='circle', style='filled')
        for i in range(self.num_node):
            for j in range(i+1, self.num_node):
                if self.adjcy_mat[i, j] > 0:
                    G.edge(str(j), str(i), arrowhead='none',
    ...
    def out_cytoscape(
        ...
        num_meta = meta_func.shape[1]
        for i in range(self.num_node):
            node_info += '{\"data\" : {\"id\":\"%s\",\"member\":\"%s\",\"count\":%d' % (
                self.node_name[i], str(list(sample_name[self.node_id_set[i]])), len(self.node_id_set[i]))
            for k in range(num_meta):
                if meta_func[0, k] in meta_data[0]:
                    meta_func_name = meta_func[0, k]
                    if meta_func[1, k][-4:] == 'RECODE':
                    ...
                    meta_data_sec = meta_data[1:, np.where(
                        meta_data[0] == meta_func_name)[0][0]]
                    node_info += vmapper.meta_func.add_meta_info(
                        meta_func[0, k], meta_data_sec, meta_func[1, k], self.node_id_set[i])
            node_info += '}},'
        node_info = node_info[:-1] + ']'
        edge_info = '\"edges\":['
            ...
            for j in range(i+1, self.num_node):
                if self.adjcy_mat[i, j] > 0:
                    edge_idx_set = np.intersect1d(
                        self.node_id_set[i], self.node_id_set[j])
                    edge_info += '{\"data\" : {\"source\":\"%s\",\"target\":\"%s\",\"name\":\"%s-%s\",\"member\":\"%s\","count":%d,\"annotation\":\"strict\"' % (
                        self.node_name[i], self.node_name[j], self.node_name[i], self.node_name[j], str(list(sample_name[edge_idx_set])), self.adjcy_mat[i, j])
                    for k in range(num_meta):
                        if meta_func[0, k] in meta_data[0]:
                            meta_func_name = meta_func[0, k]
                            if meta_func[1, k][-4:] == 'RECODE':
                            ...
                            meta_data_sec = meta_data[1:, np.where(
                                meta_data[0] == meta_func_name)[0][0]]
                            edge_info += vmapper.meta_func.add_meta_info(
                                meta_func[0, k], meta_data_sec, meta_func[1, k], edge_idx_set)
                    edge_info += '}},'
        edge_info = edge_info[:-1] + ']'
        out_cyjs.write('{%s,\"elements\":{%s,%s}}' %
...
class dmapper(mapper):
    def __init__(
        ...
        mp = mapper(data, data_p, num_cube, overlap,
                    clusterer, adaptive=adaptive)
        ...
        clstr_sub_set = [np.unique(flow_data[clstr_id_set[i]])
                         for i in range(num_clstr)]
        ...
        for i_clstr in range(num_clstr):
            ...
            for j in list(clstr_sub_set[i_clstr]):
                node_id_set[i_node] = clstr_id_set[i_clstr][np.where(
                    flow_data[clstr_id_set[i_clstr]] == j)[0]]
                id_dupl = 0
                clstr_list = [str(node_id_set[i_node]) == str(
                    node_id_set[k]) for k in range(i_node)]
                ...
                if id_dupl == 0:
                    ...
                    node_name[i_node] = '%s_%d' % (
                        str(clstr_sub_name[i_clstr]), i_clstr_sub+1)
                    i_node += 1
                    i_clstr_sub += 1
        num_node = i_node
        self.num_node = i_node
        self.adjcy_mat = np.empty([num_node, num_node], dtype=int)
        self.adjcy_mat = np.array([[len(np.intersect1d(node_id_set[i], node_id_set[j]))
                                  for i in range(num_node)] for j in range(num_node)], dtype=int)
    ...
    def out_png(
        ...
        G.attr('node')
        for i in range(self.num_node):
            i_color = np.where(flow_graph_node == self.node_flow_id[i])[
                0][0] % color_id_max + 1
            G.node(str(i), label='', fillcolor='%s' %
                   vmapper.color.color_dict[i_color][0], width='0.075', shape='circle', style='filled')
        for i in range(self.num_node):
            for j in range(self.num_node):
                if self.adjcy_mat[i, j] > 0 and i > j:
                    i_color = np.where(flow_graph_node == self.node_flow_id[i])[
                        0][0] % color_id_max + 1
                    G.edge(str(i), str(j), dir='both', arrowtail='none', arrowhead='none',
                           arrowsize='0.2', color='%s' % vmapper.color.color_dict[i_color][0])
                elif len(np.intersect1d(self.node_clstr_id[i], self.node_clstr_id[j])):
                    i_graph = np.where(flow_graph_node ==
                                       self.node_flow_id[i])[0][0]
                    j_graph = np.where(flow_graph_node ==
                                       self.node_flow_id[j])[0][0]
                    dis = flow_graph_dis[i_graph, j_graph]
                    if dis == 1:
                        G.edge(str(i), str(j), arrowhead='normal',
                               arrowsize='0.4', color='black')
                    elif dis == np.inf:
                        G.edge(str(i), str(j), arrowhead='none', arrowsize='0.2',
                               color='darkslategray', style='dashed')
                    elif dis > 1:
                        G.edge(str(i), str(j), arrowhead='empty',
    ...
    def out_cytoscape(
        ...
        num_meta = meta_func.shape[1]
        for i in range(self.num_node):
            node_info += '{\"data\" : {\"id\":\"%s\",\"member\":\"%s\",\"count\":%d,\"flow_id\":\"%s\"' % (
                self.node_name[i], str(list(sample_name[self.node_id_set[i]])), len(self.node_id_set[i]), str(self.node_flow_id[i]))
            for k in range(num_meta):
                if meta_func[0, k] in meta_data[0]:
                    meta_func_name = meta_func[0, k]
                    if meta_func[1, k][-4:] == 'RECODE':
        ...
        for i in range(self.num_node):
            for j in range(self.num_node):
                if self.adjcy_mat[i, j] > 0 and i > j:
                    edge_idx_set = np.intersect1d(
                        self.node_id_set[i], self.node_id_set[j])
                    edge_info += '{\"data\" : {\"source\":\"%s\",\"target\":\"%s\",\"name\":\"%s-%s\",\"member\":\"%s\","count":%d,\"annotation\":\"strict\",\"flow_id\":\"%s\",\"distance\":0' % (
                        self.node_name[i], self.node_name[j], self.node_name[i], self.node_name[j], str(list(sample_name[edge_idx_set])), self.adjcy_mat[i, j], str(self.node_flow_id[i]))
                    for k in range(num_meta):
                        if meta_func[0, k] in meta_data[0]:
                            meta_func_name = meta_func[0, k]
                            if meta_func[1, k][-4:] == 'RECODE':
                    ...
                    dis = flow_graph_dis[i_graph, j_graph]
                    if dis == 1:
                        edge_info += '{\"data\" : {\"source\":\"%s\",\"target\":\"%s\",\"name\":\"%s-%s\",\"member\":\"[]\","count":0,\"annotation\":\"arrow\",\"flow_id\":\"%s-%s\",\"distance\":%d' % (
                            self.node_name[i], self.node_name[j], self.node_name[i], self.node_name[j], str(self.node_flow_id[i]), str(self.node_flow_id[j]), int(dis))
                    elif dis == np.inf:
                        edge_info += '{\"data\" : {\"source\":\"%s\",\"target\":\"%s\",\"name\":\"%s-%s\",\"member\":\"[]\","count":0,\"annotation\":\"dashed\",\"flow_id\":\"%s-%s\",\"distance\":null' % (
                            self.node_name[i], self.node_name[j], self.node_name[i], self.node_name[j], str(self.node_flow_id[i]), str(self.node_flow_id[j]))
                    elif dis > 1:
                        edge_info += '{\"data\" : {\"source\":\"%s\",\"target\":\"%s\",\"name\":\"%s-%s\",\"member\":\"[]\","count":0,\"annotation\":\"two_arrow\",\"flow_id\":\"%s-%s\",\"distance\":%d' % (
                            self.node_name[i], self.node_name[j], self.node_name[i], self.node_name[j], str(self.node_flow_id[i]), str(self.node_flow_id[j]), int(dis))
                    else:
                        continue
                    for k in range(num_meta):
                        if meta_func[0, k] in meta_data[0]:
                            meta_data_sec = meta_data[1:, np.where(
                                meta_data[0] == meta_func[0, k])[0][0]]
                            edge_idx_set = np.hstack(
                                (self.node_id_set[i], self.node_id_set[j]))
                            edge_info += vmapper.meta_func.add_meta_info(
                                meta_func[0, k], meta_data_sec, meta_func[1, k], edge_idx_set)
                    edge_info += '}},'
...
def plot_flow(
    ...
    flow_data_mat = np.array(
        np.where(flow_data[1:, 1:] == '', 0, flow_data[1:, 1:]), dtype=int)
    ...
    for i in range(num_node):
        i_color = i % color_id_max
        G.node(str(i), label='%s' % (flow_data_name[i]), fillcolor='%s' % vmapper.color.color_dict[i+1][0],
               fontcolor='%s' % vmapper.color.color_dict[i+1][1], width='0.075', shape='square', style='filled')
    for i in range(num_node):
        for j in range(num_node):
            if flow_data_mat[i, j] == 1:
                G.edge(str(i), str(j), arrowhead='normal',
                       arrowsize='1', color='black')
