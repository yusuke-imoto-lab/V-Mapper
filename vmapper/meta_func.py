...
def add_meta_info(
    ...
    if func == 'list':
        ...
        meta_list_uni_count = [np.count_nonzero(
            meta_list == meta_list_uni[i]) for i in range(len(meta_list_uni))]
        percent = ['{:.0%}'.format(meta_list_uni_count[i]/len(meta_list))
                   for i in range(len(meta_list_uni))]
        ...
        meta_list_uni_count = [np.count_nonzero(
            meta_list == meta_list_uni[i]) for i in range(len(meta_list_uni))]
        max_id = np.array(np.where(meta_list_uni_count ==
                          np.max(meta_list_uni_count))[0])
        ...
        max_id = np.array(np.where(meta_list_uni_count ==
                          np.max(meta_list_uni_count))[0])
        max_percent = ['{:.0%}'.format(
            meta_list_uni_count[max_id[i]]/len(meta_list)) for i in range(len(max_id))]
        if id_light:
            info += '%s / %s' % (str(list(meta_list_uni[max_id]))[
                                 1:-1], str(list(max_percent))[1:-1])
        else:
            info += '\"%s / %s\"' % (str(list(meta_list_uni[max_id]))[
                                     1:-1], str(list(max_percent))[1:-1])
    elif func[:7] == 'average':
        info += '%0.4f' % (
            np.mean(np.array(meta_data[id_set], dtype=float), axis=0))
    elif func[:4] == 'mean':
        info += '%0.4f' % (
            np.mean(np.array(meta_data[id_set], dtype=float), axis=0))
    elif func[:6] == 'median':
        info += '%0.4f' % (
            np.median(np.array(meta_data[id_set], dtype=float), axis=0))
    elif func[:3] == 'max':
        info += '%0.4f' % (
            np.max(np.array(meta_data[id_set], dtype=float), axis=0))
    elif func[:3] == 'min':
        info += '%0.4f' % (
            np.min(np.array(meta_data[id_set], dtype=float), axis=0))
    elif func == 'union':
        meta_data[id_set]
        if id_light:
